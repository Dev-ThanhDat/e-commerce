...
const CategoriesProducts = () => {
  ...
  useEffect(() => {
    const filteredCategory = pCategories?.map((category, index) => ({
      key: pCategories.length - index,
  ...
  const onSubmit = async (data) => {
    try {
      let response;
      if (editMode) {
        response = await updatePCategorys(currentCategoryId, data.title);
      } else {
        response = await createPCategorys(data.title);
      }
      if (response.success) {
        messageApi.open({
          type: 'success',
  ...
  const handleDelete = (categoriId) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const response = await deletePCategorys(categoriId);
          if (response.success) {
            messageApi.open({
              type: 'success',
              content: response.message
